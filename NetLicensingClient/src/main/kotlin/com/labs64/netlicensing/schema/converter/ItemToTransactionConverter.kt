        ...
        source?.let {
            ...
            for (list in source.list) {
                if (Constants.Transaction.LICENSE_TRANSACTION_JOIN == list.name) {
                    ...
                    transaction.number =
                            SchemaFunction.propertyByName(list.property, Constants.Transaction.TRANSACTION_NUMBER).value
                    licenseTransactionJoin.transaction = transaction

                    val license = LicenseImpl()
                    license.number =
                            SchemaFunction.propertyByName(list.property, Constants.License.LICENSE_NUMBER).value
